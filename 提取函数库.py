...
def 提取年龄(input):
    ...
    if m:
        DOB = date(int(m.groups()[0]),int(m.groups()[1]),int(m.groups()[2]))
        result = (date.today()- DOB).days//365
        return str(result)
...
def 提取认罪态度良好(input):
    result = False
    if re.search("认罪态度良好|态度良好",input):
        result = True
    return 布尔编码(result)

def 提取认罪认罚(input):
    result = False
    if re.search("认罪|认罚|如实供述", input):
        result = True
    return 布尔编码(result)
...
def 提取前科(input):
    ...
    if re.search("无.*前科|没有.*前科|初犯",input):
        result = False
    if re.search("曾因犯|[^没]有.*犯?罪?前科|有前科劣迹|累犯|曾被判|日因犯.*罪|因犯.*罪\,于|刑满释放",input):
        result = True
    return 布尔编码(result)

def 提取累犯(input):
    result = False
    if re.search("累犯",input):
        result = True
    return 布尔编码(result)
...
def 提取盗窃数额(input):
    ...
    re_templates = [
        ...
        "经鉴定共计人民币(\d*)余?元",
        "盗窃.*共计人民币(\d*)余?元",
        "被[盗|窃|偷].*总价格为人民币(\d*)余?元",
        "累计.*人民币(\d*)余?元",
        "[盗|窃|偷][得|取].*共计人民币(\d*)余?元",
        "总价.*人民币(\d*)余?元",
    ]
    for template in re_templates:
